...
/// Create a pointer literal referencing the variable ``backing``, tagged with
/// the given opaque resource ``kind`` and either read or write access.
...
/// Open a symbolic recording session (see jit_record_begin())
extern uint32_t jitc_record_begin(JitBackend backend, const char *name);

/// Close a symbolic recording session (see jit_record_end())
extern void jitc_record_end(JitBackend backend, uint32_t value, int cleanup);
...
uint32_t jitc_var_repeat(uint32_t index, uint32_t count, size_t max_size = 0);
...
// FP16 accumulation-shadow machinery for Metal

/// Allocate or reuse a FP32 shadow accumulation buffer
extern uint32_t jitc_var_shadow_make(uint32_t target, bool &fresh);

/// Release the FP32 shadow and potentially narrow back to FP16
extern void jitc_var_shadow_narrow(uint32_t index, bool narrow);

/// Set up a deferred call to jitc_var_shadow_narrow
extern void jitc_var_shadow_setup_narrow(uint32_t se_node, uint32_t target);
#endif
