...
struct CUDAThreadState: ThreadState{
    ...
    void memset_async(void *ptr, uint32_t size,
                                   uint32_t isize, const void *src) override;
    
    /// Reduce the given array to a single value
    void reduce(VarType type, ReduceOp rtype, const void *ptr,
    ...
    void prefix_sum(VarType vt, bool exclusive, const void *in,
                                 uint32_t size, void *out) override;
    ...
    void block_copy(enum VarType type, const void *in, void *out,
                                 uint32_t size, uint32_t block_size) override;

    /// Sum over elements within blocks
    void block_sum(enum VarType type, const void *in, void *out,
                                uint32_t size, uint32_t block_size) override;
    ...
    void reduce_expanded(VarType, ReduceOp, void*, uint32_t, uint32_t) override{
