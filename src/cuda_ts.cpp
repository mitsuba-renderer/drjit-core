...
uint32_t CUDAThreadState::block_mkperm(const uint32_t *ptr, uint32_t size,
    ...
    uint32_t blocks_per_group, thread_count;
    dev.get_launch_config(&blocks_per_group, &thread_count, block_size, 1024, 1);
    ...
    if (unlikely(blocks_per_group > 65535))
        jitc_raise("jit_block_mkperm(): block_size=%u requires %u CUDA blocks "
                   "per group, exceeding the gridDim.y limit of 65535.",
                   block_size, blocks_per_group);

    uint32_t gpu_block_count = n_blocks * blocks_per_group;
    ...
    uint32_t rows_per_group = is_tiny
        ? blocks_per_group * warp_count : blocks_per_group;
    ...
    uint32_t size_per_gpu_block =
        (block_size + blocks_per_group - 1) / blocks_per_group;
    ...
    void *args_1[] = { &ptr, &buckets_1, &size, &size_per_gpu_block,
                       &bucket_count, &block_size, &blocks_per_group };

    submit_gpu(KernelType::CallReduce, this->recording_mode, phase_1,
               n_blocks, thread_count, shared_size, stream, args_1,
               nullptr, size, blocks_per_group);
    ...
    submit_gpu(KernelType::CallReduce, this->recording_mode, phase_4,
               n_blocks, thread_count, shared_size, stream, args_4,
               nullptr, size, blocks_per_group);
