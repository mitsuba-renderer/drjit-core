...
/// Generate the LLVM IR of an intersection function
...
/// Generate the PTX of an OptiX intersection program
...
/// Generate the MSL of an intersection function
extern void jitc_metal_assemble_isect(const CallData *call);

/// Reset per-kernel Metal assembly state at the start of jitc_eval()
extern void jitc_metal_assemble_reset();

/// Append an indented copy of the unformatted MSL in ``src`` to ``out``
/// (used by the PrintIR / console IR dump and the kernel history)
extern void jitc_metal_format(const char *src, size_t size, StringBuffer &out);
