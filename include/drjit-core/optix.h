 ...
 * \c jit_optix_ray_trace_new for how to use these.
 */
#if defined(__cplusplus)
enum class OptixHitObjectField: uint32_t {
    /// Whether the HitObject is for a hit or not (miss or nop)
    IsHit,

    /// Instance ID of the hit object if it is an instanced object
    InstanceId,

    /// Index of the primitive that was hit, i.e triangle ID
    PrimitiveIndex,

    /// Pointer to the SBT data buffer of the hit object
    SBTDataPointer,

    /// Ray's distance to the object what was hit
    RayTMax,

    /// The HitObject attribute at index 0
    Attribute0,

    /// The HitObject attribute at index 1
    Attribute1,

    /// The HitObject attribute at index 2
    Attribute2,

    /// The HitObject attribute at index 3
    Attribute3,

    /// The HitObject attribute at index 4
    Attribute4,

    /// The HitObject attribute at index 5
    Attribute5,

    /// The HitObject attribute at index 6
    Attribute6,

    /// The HitObject attribute at index 7
    Attribute7,
...
/**
  ...
  *
  * The outoing hit object resulting of the ``optixTraverse`` call can be
