...
int main(int argc, char **argv) {
    ...
    *last_backslash = L'\0';
    ...
    bool test_cuda = true, test_optix = true, test_llvm = true,
         test_metal = true, write_ref = false, help = false;
    ...
    try {
        ...
        uint32_t flags = 0;
        if (test_llvm)
            flags |= (1 << (uint32_t) JitBackend::LLVM);
        if (test_cuda || test_optix)
            flags |= (1 << (uint32_t) JitBackend::CUDA);
        if (test_metal)
            flags |= (1 << (uint32_t) JitBackend::Metal);
        jit_init(flags);
